...
import org.conference.model.Talk;
...
public class TalkBean implements Serializable {
	...
	@PersistenceContext(unitName = "conference-persistence-unit", type = PersistenceContextType.EXTENDED)
	...
	private Predicate[] getSearchPredicates(Root<Talk> root) {
		...
		if (room != null && !"".equals(room)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("room")),
					'%' + room.toLowerCase() + '%'));
		}
	...
	public List<Talk> getAll() {
		...
		CriteriaQuery<Talk> criteria = this.entityManager.getCriteriaBuilder()
				.createQuery(Talk.class);
	...
	public Converter getConverter() {
		...
		final TalkBean ejbProxy = this.sessionContext
				.getBusinessObject(TalkBean.class);
